...
def train_with_train_and_test_files(train_paths, test_paths, model_name_prefix="fusion",
    ...
    """
    print("[train] Preparing web payload anomaly training data...")
    X_train_df, Y_train_ser, features_train = prepare_from_paths(train_paths)
    if not (Y_train_ser == 0).any():
        raise RuntimeError(
            "[train] No NORMAL (label=0) samples found. "
            "Anomaly detection requires normal-only or mostly-normal training data."
        )
